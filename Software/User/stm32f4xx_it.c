  ...
  for (int j = 0; j < 8; j++)
  {
    ...
    if (StdId == (0x300 + Driver[j].command.canId))
    {
      ...
      case 0x00004F4D: //MO
        ...
        break;
			case 0x00004D55:
				if(Msg1.data32[1] == 2)  //Spd_Mode
				{Driver[j].unitMode = SPEED_CONTROL_MODE;}
				else 
				if(Msg1.data32[1] == 5)  //Pos_Mode
				{Driver[j].unitMode = POSITION_CONTROL_MODE;}
				else if(Msg1.data32[1] == 6)
				{Driver[j].unitMode = HOMING_MODE;}
				break;
